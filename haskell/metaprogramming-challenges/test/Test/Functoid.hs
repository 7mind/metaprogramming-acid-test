...
spec = describe "Functoid" $ do
    describe "Unary functoid" $ do
        ...
        it "can be invoked" $
            invoke @(Int -> Int) doubleFunctoid 21 `shouldBe` 42
    ...
    describe "Binary functoid" $ do
        ...
        it "can be invoked" $
            invoke @(Int -> Int -> Int) addFunctoid 10 32 `shouldBe` 42
    ...
    describe "Ternary functoid" $ do
        ...
        it "can be invoked" $
            invoke @(String -> Int -> Bool -> String) combineFunctoid "Alice" 30 True `shouldBe` "Alice:30:True"
